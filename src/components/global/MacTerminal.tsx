...
export default function MacTerminal({ isOpen, onClose }: MacTerminalProps) {
  ...
  const welcomeMessage = `Bienvenido a Mi Portafolio

Nombre: ${userConfig.name}
Rol: ${userConfig.role}
Ubicación: ${userConfig.location}
...
My education:
${userConfig.education.map(edu => `- ${edu.degree}${edu.major ? ` in ${edu.major}` : ''} — ${edu.institution}, ${edu.location} (${edu.year})`).join('\n')}
    ...
    try {
      ...
      setChatHistory((prev) => ({
        ...
        messages: [
          ...
          {
            role: 'assistant',
            content: `Tuve un problema al procesar eso. Por favor escríbeme a ${userConfig.contact.email}`,
          },
        ],
      }));
    } finally {

...
export default function MissionControl({ isOpen, onClose, activeApps, onAppClick, onAppClose }: MissionControlProps) {
  ...
  const apps = [
    { id: 'github' as const, name: 'Proyectos GitHub', icon: BsGithub, color: 'from-black to-black/60', active: activeApps.github },
    { id: 'notes' as const, name: 'Notas', icon: BsStickyFill, color: 'from-yellow-600 to-yellow-400', active: activeApps.notes },
    { id: 'terminal' as const, name: 'Terminal', icon: RiTerminalFill, color: 'from-black to-black/60', active: activeApps.terminal },
    { id: 'resume' as const, name: 'CV', icon: BsFilePdf, color: 'from-red-600 to-red-400', active: activeApps.resume },
    ...
    <div className="fixed inset-0 z-[90]" role="dialog" aria-modal="true" aria-label="Mission Control">
      ...
      <div className="relative h-full flex flex-col items-center justify-center p-8">
        ...
        {activeWindows.length === 0 ? (
          <p className="text-gray-400 text-lg">Sin ventanas abiertas</p>
        ) : (
        ...
        <p className="text-gray-400 text-sm mt-8">Presiona Esc o haz clic afuera para cerrar</p>
      </div>
    </div>
  );
}

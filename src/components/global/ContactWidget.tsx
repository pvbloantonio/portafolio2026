      ...
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-label="Contact form" className="relative w-full md:max-w-lg bg-gray-900/95 text-white rounded-2xl shadow-2xl p-6 border border-white/10">
        <div className="flex items-start justify-between gap-6">
          <div>
            ...
            <p className="text-sm text-gray-300 mt-1">¿Tienes una pregunta u oportunidad? Envíame un mensaje.</p>
          </div>
        ...
        {success ? (
          <div className="mt-6 bg-green-600/10 border border-green-500/30 text-green-200 rounded-lg p-4">
            <p>¡Gracias! Tu mensaje fue enviado. Te responderé a <span className="font-medium">{email}</span>.</p>
          </div>
        ) : (
          <form className="mt-6 space-y-4" onSubmit={send}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input value={name} onChange={(e) => setName(e.target.value)} className="mt-1 w-full rounded-lg bg-white/5 border border-white/10 px-3 py-2 outline-none focus:ring-2 focus:ring-white/40" placeholder="Tu nombre" disabled={disabled} />
              </div>
              <div>
                <label className="text-sm text-gray-300">Email</label>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1 w-full rounded-lg bg-white/5 border border-white/10 px-3 py-2 outline-none focus:ring-2 focus:ring-white/40" placeholder="tu@ejemplo.com" disabled={disabled} />
              </div>
            </div>
            ...
            <div>
              ...
              <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={5} className="mt-1 w-full rounded-lg bg-white/5 border border-white/10 px-3 py-2 outline-none focus:ring-2 focus:ring-white/40 resize-y" placeholder="¿En qué puedo ayudarte?" disabled={disabled} />
            </div>
            {error && (
            ...
            <div className="flex items-center justify-between">
              ...
              <a href={`mailto:${userConfig.contact.email}`} className="text-sm text-gray-300 hover:text-white underline underline-offset-2">Envíame un email directo</a>
            </div>
          </form>
        )}
      </div>
